...
| `OpenFoundationModelsParityTests` | `OpenFoundationModels` (this package) | `qwen3.5:9b` via Ollama |
...
swift test --filter OpenFoundationModelsParityTests
```

Both currently pass 14/14. The Apple run defines correct behavior; this library
must match it, test for test.

Environment overrides for the open side: `PARITY_BASE_URL` (default
